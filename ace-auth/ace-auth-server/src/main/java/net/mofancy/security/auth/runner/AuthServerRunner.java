package net.mofancy.security.auth.runner;

import net.mofancy.security.auth.common.util.jwt.RsaKeyHelper;
...
public class AuthServerRunner implements CommandLineRunner {
    ...
    private static final String REDIS_APP_PRI_KEY = "APP:AUTH:JWT:PRI";
    private static final String REDIS_APP_PUB_KEY = "APP:AUTH:JWT:PUB";
    ...
    public void run(String... args) throws Exception {
        if (redisTemplate.hasKey(REDIS_USER_PRI_KEY)&&redisTemplate.hasKey(REDIS_USER_PUB_KEY)
                &&redisTemplate.hasKey(REDIS_APP_PRI_KEY)&&redisTemplate.hasKey(REDIS_APP_PUB_KEY)
            ...
            keyConfiguration.setAppPriKey(RsaKeyHelper.toBytes(redisTemplate.opsForValue().get(REDIS_APP_PRI_KEY).toString()));
            keyConfiguration.setAppPubKey(RsaKeyHelper.toBytes(redisTemplate.opsForValue().get(REDIS_APP_PUB_KEY).toString()));
            ...
            keyMap = RsaKeyHelper.generateKey(keyConfiguration.getAppSecret());
            keyConfiguration.setAppPriKey(keyMap.get("pri"));
            keyConfiguration.setAppPubKey(keyMap.get("pub"));
            redisTemplate.opsForValue().set(REDIS_APP_PRI_KEY, RsaKeyHelper.toHexString(keyMap.get("pri")));
            redisTemplate.opsForValue().set(REDIS_APP_PUB_KEY, RsaKeyHelper.toHexString(keyMap.get("pub")));
