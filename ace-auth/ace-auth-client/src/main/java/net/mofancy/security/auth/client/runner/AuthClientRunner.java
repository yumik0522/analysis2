package net.mofancy.security.auth.client.runner;
...
import net.mofancy.security.auth.client.feign.ServiceAuthFeign;
...
public class AuthClientRunner implements CommandLineRunner {
    ...
    @Scheduled(cron = "0 0/1 * * * ?")
    public void refreshAppPubKey(){
        BaseResponse resp = serviceAuthFeign.getAppPublicKey(serviceAuthConfig.getClientId(), serviceAuthConfig.getClientSecret());
        if (resp.getStatus() == HttpStatus.OK.value()) {
            ObjectRestResponse<byte[]> appResponse = (ObjectRestResponse<byte[]>) resp;
            this.appAuthConfig.setPubKeyByte(appResponse.getData());
        }
    }
